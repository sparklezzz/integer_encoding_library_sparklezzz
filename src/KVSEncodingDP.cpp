...
KVSEncodingDP::KVSEncodingDP(const uint32_t *lens,
...
KVSEncodingDP::~KVSEncodingDP() throw() {}

void KVSEncodingDP::computePartition(const std::vector<uint32_t>& seq,
...

  {
    ...
    for (uint64_t i = 1; i <= seq.size(); i++) {
      ...
      for (maxB = 0, l = 0, g = 0, j = i - 1;
        ...
        /* Caluculate costs */
        // use 4 multiples of data
        if (aligned_) // aligned to byte, instead of 32-bit word
          ccost = costs[j] + 32 * DIV_ROUNDUP((i - j) * maxB, 8) + cost;
        else
          ccost = costs[j] + 4 * (i - j) * maxB + cost;
