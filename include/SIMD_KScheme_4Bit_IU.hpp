/*
 * SIMD_KScheme_4Bit_IU.hpp
 *
 *  SIMD_KScheme_4Bit_Incomplete Unary
 *
 *  Created on: 2013-3-2
 *      Author: zxd
 */

#ifndef SIMD_KSCHEME_4BIT_IU_HPP_
#define SIMD_KSCHEME_4BIT_IU_HPP_
...
#include "VarByte.hpp"
#include <iostream>
#include "simd_kscheme_4bit_iu_unpack.hpp"
...
class SIMD_KScheme_4Bit_IU: public Compressor {
public:
	SIMD_KScheme_4Bit_IU() {
	}
	virtual ~SIMD_KScheme_4Bit_IU() {
	}
...
template<typename T>
int SIMD_KScheme_4Bit_IU::encode(char* des, const T* src, uint32_t encodeNum) {
	if (encodeNum < 4) {
...
template<typename T>
int SIMD_KScheme_4Bit_IU::decode(T* des, const char* src, uint32_t decodeNum) {
	if (decodeNum < 4) {
...
template<typename T>
int SIMD_KScheme_4Bit_IU::encodeBlock(char* des, const T* src, uint32_t encodeNum) {
...
/*
template<typename T>
int SIMD_KScheme_4Bit_IU::decodeBlock(T* des, const char* src, uint32_t decodeNum) {
...
int SIMD_KScheme_4Bit_IU::decodeBlock(T* des, const char* src, uint32_t decodeNum) {
	...
	simd_kscheme_4bit_iu_unpack_prepare<T>();

	uint32_t decoded = 0;

	while (decoded < decodeNum) {
		// get a pack of unary descriptor
		const SIMDK4IUUnpackInfo &unpackInfo = SIMDK4IUUnpackInfoArr[*modeSegPos];
...
#endif /* SIMD_KSCHEME_4BIT_IU_HPP_ */
