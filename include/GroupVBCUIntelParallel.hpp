/*
 * GroupVBCUIntelParallel.hpp
 *
 *  Created on: 2013-1-25
 *      Author: zxd
 */

#ifndef GROUPVBCUINTELPARALLEL_HPP_
#define GROUPVBCUINTELPARALLEL_HPP_

#ifdef USE_SSE_INSTRUCTION

#include "Compressor.hpp"
#include "GvintCompUnaryUnpack.hpp"
#include "GvintCompUnaryUnpackIntelParallel.hpp"
#include "VarByte.hpp"
#include <iostream>
...
class GroupVBCUIntelParallel: public Compressor {
public:
	GroupVBCUIntelParallel() {
	}
	virtual ~GroupVBCUIntelParallel() {
	}
...
public:
	...
	virtual std::string getCompressorName() {
		return "group_var_byte_comp_unary_intel";
	}
...
template<typename T>
int GroupVBCUIntelParallel::encode(char* des, const T* src, uint32_t encodeNum) {
	int compLen = 0;
...
template<typename T>
int GroupVBCUIntelParallel::decode(T* des, const char* src, uint32_t decodeNum) {
	int decodeLen = 0;
	decodeLen = decodeBlock(des, src, decodeNum);
	return decodeLen;
}
template<typename T>
int GroupVBCUIntelParallel::encodeBlock(char* des, const T* src, uint32_t encodeNum) {
	int compSize = 0;
	uint8_t maskByte;
	int groupLen = 0;
	uint8_t buffer[16];
...
template<typename T>
int GroupVBCUIntelParallel::decodeBlock(T* des, const char* src, uint32_t decodeNum) {
...
#endif /* GROUPVBCUINTELPARALLEL_HPP_ */
