/*
 * KGammaSeq.hpp
 * principle: see paper: Fast Integer Compression using SIMD Instructions
 *
 *  Created on: 2013-3-2
 *      Author: zxd
 */

#ifndef KGAMMASEQ_HPP_
#define KGAMMASEQ_HPP_
...
class KGammaSeq: public Compressor {
public:
	KGammaSeq() {
	}
	virtual ~KGammaSeq() {
...
template<typename T>
int KGammaSeq::encode(char* des, const T* src, uint32_t encodeNum) {
	int compLen = 0;
...
template<typename T>
int KGammaSeq::decode(T* des, const char* src, uint32_t decodeNum) {
	int decodeLen = 0;
...
template<typename T>
int KGammaSeq::encodeBlock(char* des, const T* src, uint32_t size) {
...
template<typename T>
int KGammaSeq::decodeBlock(T* des, const char* src, uint32_t size) {
