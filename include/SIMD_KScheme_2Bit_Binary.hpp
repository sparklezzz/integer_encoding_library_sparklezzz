/*
 * SIMD_KScheme_2Bit_Binary.hpp
 *
 *  Created on: 2013-3-2
 *      Author: zxd
 */

#ifndef SIMD_KSCHEME_2BIT_BINARY_HPP_
#define SIMD_KSCHEME_2BIT_BINARY_HPP_
...
#include "VarByte.hpp"
#include <iostream>
#include "simd_kscheme_2bit_binary_unpack.hpp"
...
class SIMD_KScheme_2Bit_Binary: public Compressor {
public:
	SIMD_KScheme_2Bit_Binary() {
	}
	virtual ~SIMD_KScheme_2Bit_Binary() {
	}
...
public:
	...
	virtual std::string getCompressorName() {
		return "SIMD_KScheme_2Bit_Binary";
	}
...
template<typename T>
int SIMD_KScheme_2Bit_Binary::encode(char* des, const T* src, uint32_t encodeNum) {
	if (encodeNum < 4) {
...
template<typename T>
int SIMD_KScheme_2Bit_Binary::decode(T* des, const char* src, uint32_t decodeNum) {
	if (decodeNum < 4) {
...
template<typename T>
int SIMD_KScheme_2Bit_Binary::encodeBlock(char* des, const T* src, uint32_t encodeNum) {
	...
	*desInt = 4 + (quadEncodeNum + 1) / 2;	// record the begin position of data segment
	...
	for (uint32_t i=0; i<encodeNum; i+=4) {
		...
		*modeSegPos |= (twoBitCount-1) << ((quadIdx & 0x1) << 2);
		quadIdx ++;
		if (!(quadIdx & 0x1)) {
			modeSegPos ++;
		}

		uint32_t bitCount = twoBitCount << 1;
...
template<typename T>
int SIMD_KScheme_2Bit_Binary::decodeBlock(T* des, const char* src, uint32_t decodeNum) {
	...
	simd_kscheme_2bit_binary_unpack_prepare<T>();

	uint32_t decoded = 0;
	uint32_t curBitOffset = 0;

	while (decoded + 4 < decodeNum) {
		const SIMDK2BUnpackInfo &unpackInfo = SIMDK2BUnpackInfoArr[*modeSegPos];
	...
	while (decoded < decodeNum) {
		for (int i=0; i<2 && decoded < decodeNum; i++) {
			uint32_t twoBitCount = 1 + (((*modeSegPos) >> (i<<2)) & 0xf);
			uint32_t bitCount = twoBitCount << 1;
			uint32_t bitLeftInUInt = 32 - curBitOffset;
			for	(uint32_t k=0; k<4; k++) {
...
#endif /* SIMD_KSCHEME_2BIT_BINARY_HPP_ */
