...
#ifndef SIMD_KVARBYTE_CU_HPP_
#define SIMD_KVARBYTE_CU_HPP_
...
class SIMD_KVarByte_CU: public Compressor {
public:
	SIMD_KVarByte_CU() {
	}
	virtual ~SIMD_KVarByte_CU() {
	}
...
template<typename T>
int SIMD_KVarByte_CU::encode(char* des, const T* src, uint32_t encodeNum) {
	if (encodeNum < 4) {
...
template<typename T>
int SIMD_KVarByte_CU::decode(T* des, const char* src, uint32_t decodeNum) {
	if (decodeNum < 4) {
...
template<typename T>
int SIMD_KVarByte_CU::encodeBlock(char* des, const T* src, uint32_t encodeNum) {
...
template<typename T>
int SIMD_KVarByte_CU::decodeBlock(T* des, const char* src, uint32_t decodeNum) {
