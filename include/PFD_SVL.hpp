...
class PFD_SVL: public PForDeltaBase {
public:
	PFD_SVL(float expRatio = 0.1);
...
template<typename T>
int PFD_SVL::encode(char* des, const T* src, uint32_t encodeNum) {
	int compLen = 0;
...
template<typename T>
int PFD_SVL::decode(T* des, const char* src, uint32_t decodeNum) {
	int decodeLen = 0;
...
template<typename T>
int PFD_SVL::encodeBlock(char* des, const T* src) {
	uint32_t expNum = 0;
	int compSize = 0;
...
template<typename T>
int PFD_SVL::decodeBlock(T* des, const char* src) {
...
template<typename T>
int PFD_SVL::pfor_encode(char *des, const T *src, int useBits,
		uint32_t& expNum) {
	...
	if (expNum <= m_expRatio * m_blockSize) {
		...
		compSize += needWord * sizeof(uint32_t);
		if (expBit == 8) {
			pack_se8<uint32_t>(desInt, expNumArr, expNum);
		}
		else if (expBit == 16){
			pack_se16<uint32_t>(desInt, expNumArr, expNum);
		}
		else {
