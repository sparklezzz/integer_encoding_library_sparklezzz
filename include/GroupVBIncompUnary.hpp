/*
 * GroupVBIncompUnary.hpp
 *
 *  Created on: 2013-1-22
 *      Author: zxd
 */

#ifndef GROUPVBINCOMPUNARY_HPP_
#define GROUPVBINCOMPUNARY_HPP_
...
class GroupVBIncompUnary: public Compressor {
public:
	GroupVBIncompUnary() {
	}
	virtual ~GroupVBIncompUnary() {
	}
...
public:
	...
	virtual std::string getCompressorName() {
		return "group_var_byte_incomp_unary";
	}
...
template<typename T>
int GroupVBIncompUnary::encode(char* des, const T* src, uint32_t encodeNum) {
	int compLen = 0;
...
template<typename T>
int GroupVBIncompUnary::decode(T* des, const char* src, uint32_t decodeNum) {
	int decodeLen = 0;
	decodeLen = decodeBlock(des, src, decodeNum);
	return decodeLen;
}
template<typename T>
int GroupVBIncompUnary::encodeBlock(char* des, const T* src, uint32_t encodeNum) {
	int compSize = 0;
	uint8_t maskByte;
	int groupLen = 0;
	uint8_t buffer[16];
...
template<typename T>
int GroupVBIncompUnary::decodeBlock(T* des, const char* src, uint32_t decodeNum) {
