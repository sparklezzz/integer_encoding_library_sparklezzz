/**
 * filename:SIMD_Group_PFD_3.hpp
 * @brief:
 * @date:2014-02-28
 * @author:zxd
 *
 * encoding format for SIMD_Group_PFD_3:
...
#ifndef SIMD_GROUP_PFD_3_HPP_
#define SIMD_GROUP_PFD_3_HPP_
...
class SIMD_Group_PFD_3: public PForDeltaBase {
public:
	SIMD_Group_PFD_3(float expRatio = 0.1);
...
private:
	...
	template<typename T>
	uint32_t getBestB(const T *src, int blockSize, bool isOriginalArr);
...
template<typename T>
int SIMD_Group_PFD_3::encode(char* des, const T* src, uint32_t encodeNum) {
	int compLen = 0;
...
template<typename T>
int SIMD_Group_PFD_3::decode(T* des, const char* src, uint32_t decodeNum) {
	int decodeLen = 0;
...
template<typename T>
int SIMD_Group_PFD_3::encodeAlign(char* des, const T* src, uint32_t encodeNum) {
	if (encodeNum == 0)
		return 0;
...
template<typename T>
int SIMD_Group_PFD_3::decodeAlign(T* des, const char* src, uint32_t decodeNum) {
	if (decodeNum == 0)
		return 0;
...
template<typename T>
int SIMD_Group_PFD_3::encodeExp(char *des, const T *dummy) {
...
template<typename T>
int SIMD_Group_PFD_3::decodeExp(const T* src) {
...
template<typename T>
int SIMD_Group_PFD_3::encodeBlock(char* des, const T* src) {
	...
	bits = getBestB(maxQuadIntArr, quadEncodeNum, false);
...
template<typename T>
int SIMD_Group_PFD_3::decodeBlock(T* des, const char* src, uint32_t blockHeader) {
...
template<typename T>
uint32_t SIMD_Group_PFD_3::getBestB(const T *src, int blockSize, bool isOriginalArr) {
	uint32_t freqs[33];
	...
	uint32_t logIntervalPad = isOriginalArr ? 0 : 2;

	uint32_t bestB = 32;
        uint32_t bestCost = bestB * m_blockSize;
	uint32_t bestExpNum = 0;
	uint32_t expNum = 0;
	while (freqs[bestB] == 0)
		bestB--;
	uint32_t maxB = bestB;
	uint32_t expB = (maxB <= 8) ? 8 : (maxB <= 16 ? 16 : 32);

	for (int b = maxB -1; b >= 0; --b) {
		expNum += freqs[b + 1];

		/*
 		* We must consider the insertion of added exception offset
 		* Suppose the original exceptions are uniformly distributed
		*/
		uint32_t expInterval = m_blockSize / expNum;
		uint32_t logExpInterval = asmbits(expInterval) + logIntervalPad;	
		uint32_t factor = 0;
		if (logExpInterval > b)
			factor = pow(2, logExpInterval - (double)b) - 1;

		uint32_t thisCost = b * m_blockSize + expB * expNum * (1 + factor);
...
/*
template<typename T>
bool SIMD_Group_PFD_3::pfor_fake_encode(const T *src, int useBits, int blockSize) {
	uint32_t expNum = 0;
...
template<typename T>
int SIMD_Group_PFD_3::pfor_encode(char *des, const T *src, int useBits,
		uint32_t& expNum) {
