/*
 * SIMD_KVarByte_IU.hpp
 *
 *  SIMD_KVarByte_Incomplete Unary
 *
 *  Created on: 2013-3-2
 *      Author: zxd
 */

#ifndef SIMD_KVARBYTE_IU_HPP_
#define SIMD_KVARBYTE_IU_HPP_
...
#include "simd_kvint_iu_unpack.hpp"
#include "simd_kvint_iu_pack.hpp"
...
class SIMD_KVarByte_IU: public Compressor {
public:
	SIMD_KVarByte_IU() {
	}
	virtual ~SIMD_KVarByte_IU() {
	}
...
template<typename T>
int SIMD_KVarByte_IU::encode(char* des, const T* src, uint32_t encodeNum) {
	if (encodeNum < 4) {
...
template<typename T>
int SIMD_KVarByte_IU::decode(T* des, const char* src, uint32_t decodeNum) {
	if (decodeNum < 4) {
...
/*
template<typename T>
int SIMD_KVarByte_IU::encodeBlock(char* des, const T* src, uint32_t encodeNum) {
	...
	while (encoded < encodeNum) {
		...
		// pad the last descriptor as needed
		if (i == encodeNum && controlBitOffset < 8) {
			uint8_t pad = (0xff >> controlBitOffset) << controlBitOffset;
			*tmpModeSegIdx |= pad;
		}

		// compress the pack
		KVIntIUPackInfo packInfo = KVIntIUPackInfoArr[*tmpModeSegIdx];
		(packInfo.m_subFunc)(dataSegPosInt, (const uint32_t*)src);

		src += packInfo.m_intNum;
		encoded += packInfo.m_intNum;
		dataSegPosInt += packInfo.m_codewordNum;
		tmpModeSegIdx ++;
	}
	...
	for (uint32_t i=0; i<encodeNum; i+=4) {
		...
		if (controlBitOffset + byteCount > 8) {	//start new mode and new data seg
			...
			*tmpModeSegIdx |= pad;
			...
			//write back to the memory in advance
...
template<typename T>
int SIMD_KVarByte_IU::decodeBlock(T* des, const char* src, uint32_t decodeNum) {
	...
	simd_kvint_iu_unpack_prepare<T>();

	uint32_t decoded = 0;

	while (decoded < decodeNum) {
		// get a pack of unary descriptor
		const KVIntIUUnpackInfo &unpackInfo = KVIntIUUnpackInfoArr[*modeSegPos];
		...
		modeSegPos ++;
		wordPos += unpackInfo.m_codewordNum;
	}
...
/*
 // bad: 50% slower than sequential version
template<typename T>
int SIMD_KVarByte_IU::decodeBlock(T* des, const char* src, uint32_t decodeNum) {
