...
#ifndef SIMD_KAFOR_HPP_
#define SIMD_KAFOR_HPP_
...
class SIMD_KAFOR: public Compressor {
public:
...
private:
	...
	template<typename T>
	uint32_t chooseBestConfig(const T *src, SIMD_KAFOR::SIMD_KAFORConfig configArr[]);
...
template<typename T>
int SIMD_KAFOR::encode(char* des, const T* src, uint32_t encodeNum) {
	uint32_t compLen = 0;
	uint32_t encoded = 0;
...
template<typename T>
int SIMD_KAFOR::decode(T* des, const char* src, uint32_t decodeNum) {
...
template<typename T>
int SIMD_KAFOR::encodeBlock(char* des, const T* src, uint32_t encodeNum) {

	SIMD_KAFOR::SIMD_KAFORConfig configArr[6];
...
template<typename T>
int SIMD_KAFOR::decodeBlock(T* des, const char* src, uint32_t decodeNum) {
...
template<typename T>
uint32_t SIMD_KAFOR::chooseAndSaveBestConfig(char *des, const T *maxQuadArr, uint32_t maxQuadNum, SIMD_KAFORConfig tmpConfigArr[]) {
...
template<typename T>
uint32_t SIMD_KAFOR::decodeAllFrames(T* des, const char* src, const char* frameHeaderArr, uint32_t numFrames) {
...
template<typename T>
uint32_t SIMD_KAFOR::chooseBestConfig(const T *src, SIMD_KAFOR::SIMD_KAFORConfig configArr[]) {
...
template<typename T>
uint32_t SIMD_KAFOR::encodeAllFrames(char *des, const T *src, const char* frameHeaderArr, uint32_t numFrames) {
	uint32_t *desInt = (uint32_t*)des;
	uint32_t byteOffset = 0;	// 0 to 3
	for (uint32_t i=0; i<numFrames; ++i) {
		uint8_t frameHeader = (uint8_t)frameHeaderArr[i];
