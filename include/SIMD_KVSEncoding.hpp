...
#ifndef SIMD_KVSEncoding_HPP_
#define SIMD_KVSEncoding_HPP_
...
#include "SIMD_KVSEncoding_unpack.hpp"
#include "encoding_internals.hpp"

namespace paradise {
namespace index {

class SIMD_KVSEncoding: public Compressor {
public:
...
private:
	...
	const static uint32_t SIMD_KVSE_LOGLEN;
	const static uint32_t SIMD_KVSE_LOGLOG;

	const static uint32_t SIMD_KVSE_LOGDESC;

	const static uint32_t SIMD_KVSE_LENS_LEN;
	const static uint32_t SIMD_KVSE_LOGS_LEN;
	const static uint32_t SIMD_KVSE_LEN;

	//const static uint32_t SIMD_KVSE_LENS[16];
	const static uint32_t SIMD_KVSE_LENS[8];

	const static uint32_t SIMD_KVSE_REMAPLOGS[33];

	const static uint32_t SIMD_KVSE_CODELENS[65];

	const static uint32_t SIMD_KVSE_CODELOGS[33];
...
template<typename T>
int SIMD_KVSEncoding::encode(char* des, const T* src, uint32_t encodeNum) {
	if (encodeNum < 4) {
...
template<typename T>
int SIMD_KVSEncoding::decode(T* des, const char* src, uint32_t decodeNum) {
	if (decodeNum < 4) {
...
template<typename T>
int SIMD_KVSEncoding::encodeBlock(char* des, const T* src, uint32_t encodeNum) {
	...
	for (uint64_t i = 0; i < k; i++)
		logs.push_back(SIMD_KVSE_REMAPLOGS[32 - MSB32(maxQuadIntArr[i])]);
	ASSERT(logs.size() == k);
	ASSERT(parts.size() == 0);
	vdp_->computePartition(logs, &parts, SIMD_KVSE_LOGLEN + SIMD_KVSE_LOGLOG);
...
template<typename T>
int SIMD_KVSEncoding::decodeBlock(T* des, const char* src, uint32_t decodeNum) {
...
template<typename T>
uint32_t SIMD_KVSEncoding::encodeAllFrames(char *des, const T *src, const std::vector<uint32_t>& logs,
            const std::vector<uint32_t> &parts){
	ASSERT(parts.size() > 1);
	...
	for (uint64_t i = 0; i < numFrames; i++) {
		...
		/*uint32_t idx = 0;
		for (; idx < ARRAYSIZE(SIMD_KVSE_LENS); idx++) {
			if (parts[i + 1] - parts[i] == SIMD_KVSE_LENS[idx])
			break;
		}*/
		int frameLenIdx = SIMD_KVSE_CODELENS[parts[i + 1] - parts[i]];
		if (frameLenIdx == -1) {
			std::cout << "wrong frame length: " << (parts[i + 1] - parts[i]) << std::endl;
		}

		uint8_t frameHeader = (frameLenIdx << SIMD_KVSE_LOGLOG) + SIMD_KVSE_CODELOGS[maxB];
...
template<typename T>
uint32_t SIMD_KVSEncoding::decodeAllFrames(T* des, const char* src, const char* frameHeaderArr, uint32_t numFrames) {

	const uint32_t *srcInt = (const uint32_t *)src;
	uint32_t byteOffset = 0;	// 0 to 3
	for (uint32_t i=0; i<numFrames; ++i) {
		uint8_t frameHeader = (uint8_t)frameHeaderArr[i];
		uint32_t SIMD_KVSEncodingUnpackInfoIdx = (((uint32_t)frameHeader) << 2) + byteOffset;
		if (SIMD_KVSEncodingUnpackInfoIdx >= 1024)
			std::cout << "wrong SIMD_KVSEUnpackInfoIdx!" << SIMD_KVSEncodingUnpackInfoIdx << std::endl;

		const SIMD_KVSEUnpackInfo &info = SIMD_KVSEUnpackInfoArr[SIMD_KVSEncodingUnpackInfoIdx];
		if (byteOffset << 3 != info.m_offset)
			std::cout << "wrong info offset!" << (byteOffset<<3) << " " << info.m_offset << std::endl;
