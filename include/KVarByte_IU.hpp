 ...
 * KVarByte_IU.hpp
 *
 *  KVarByte_Incomplete Unary
 *
 *  Created on: 2013-3-2
 *      Author: zxd
 */

#ifndef KVARBYTE_IU_HPP_
#define KVARBYTE_IU_HPP_
...
#include "VarByte.hpp"
#include <iostream>
#include "kscheme_8bit_iu_unpack.hpp"
...
class KVarByte_IU: public Compressor {
public:
	KVarByte_IU() {
	}
	virtual ~KVarByte_IU() {
...
template<typename T>
int KVarByte_IU::encode(char* des, const T* src, uint32_t encodeNum) {
	if (encodeNum < 4) {
...
template<typename T>
int KVarByte_IU::decode(T* des, const char* src, uint32_t decodeNum) {
	if (decodeNum < 4) {
...
template<typename T>
int KVarByte_IU::encodeBlock(char* des, const T* src, uint32_t encodeNum) {
	...
	// preserved bytes for modeseg: 2 * (quadEncodeNum) / 2 = quadEncodeNum
...
template<typename T>
int KVarByte_IU::decodeBlock(T* des, const char* src, uint32_t decodeNum) {
	...
	while (decoded < decodeNum) {
		...
		const KS8IUUnpackInfo &unpackInfo = KS8IUUnpackInfoArr[*modeSegPos];
