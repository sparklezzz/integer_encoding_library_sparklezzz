/*
 * unpackIntelParallelIntelParallel.hpp
 * block的块解压，每次解压32个数
 * 需要Intel的CPU
 */

#ifndef UNPACKINTELPARALLEL_HPP_
#define UNPACKINTELPARALLEL_HPP_
...
namespace index {
template<typename T>
void unpackIntelParallel0(T * des, const uint32_t *src, int blockSize) {
	int i;
	for (i = 0; i < blockSize; i++)
		des[i] = 0;
}

static const struct mask_1_0{	//for ax, cx, ex, gx or bx, dx, fx, gx (x=0,1,2,3)
int8_t a[16];
}mask_1_0 __attribute__((aligned(16))) = {{3,0xff,0xff,0xff,2,0xff,0xff,0xff,1,0xff,0xff,0xff,0,0xff,0xff,0xff},};
static const struct and_1_0{
int16_t a[8];
}and_1_0 __attribute__((aligned(16))) = {{0x1,0,0x1,0,0x1,0,0x1,0},};

template<typename T>
void unpackIntelParallel1(T* dest, const uint32_t *src, int blockSize)
{
	__asm__("movdqa %0,%%xmm0\n"//mask_0
			"movdqa %1,%%xmm1\n"//and_0
			 ::"m" (mask_1_0.a[0]),"m" (and_1_0.a[0]));

	for (int i = 32; i <= blockSize; i += 32, src += 1,dest+=32)
	{
...
/*
template<typename T>
void unpackIntelParallel1(T * des, const uint32_t *src, int blockSize) {
	int i;
...
static const struct mask_2_0{
int8_t a[16];
}mask_2_0 __attribute__((aligned(16))) = {{3,0xff,0xff,0xff,2,0xff,0xff,0xff,1,0xff,0xff,0xff,0,0xff,0xff,0xff},};
static const struct mask_2_1{
int8_t a[16];
}mask_2_1 __attribute__((aligned(16))) = {{7,0xff,0xff,0xff,6,0xff,0xff,0xff,5,0xff,0xff,0xff,4,0xff,0xff,0xff},};
static const struct and_2_0{
int16_t a[8];
}and_2_0 __attribute__((aligned(16))) = {{0x3,0,0x3,0,0x3,0,0x3,0},};

template <typename T>
void unpackIntelParallel2(T* dest, const uint32_t *src, int blockSize)
{
	__asm__("movdqa %0,%%xmm0\n"//mask_0
			"movdqa %1,%%xmm1\n"//mask_1
			"movdqa %2,%%xmm2\n"//and_0
			 ::"m" (mask_2_0.a[0]),"m" (mask_2_1.a[0]),"m" (and_2_0.a[0]));

	for (int i = 32; i <= blockSize; i += 32, src += 2,dest+=32)
	{
		__asm__ volatile("prefetchnta %0"::"m" (src[0]));
		__asm__("movdqu %8,%%xmm5\n"            //src[0]->xmm5
		"movdqa %%xmm5,%%xmm6\n"                //save for future aligned move
		"movdqa %%xmm5,%%xmm7\n"
		"movdqa %%xmm5,%%xmm8\n"
		...
		"movdqa %%xmm5,%%xmm15\n"				//save for e,f,g,h
				...
				//shift 6, 4, 2, 0
		"pshufb %%xmm0,%%xmm5\n"                //shuffle a0,b0,c0,d0->xmm5
		"psrld $6,%%xmm5\n"
		"pand %%xmm2,%%xmm5\n"
		"pshufb %%xmm0,%%xmm6\n"                //shuffle a1,b1,c1,d1->xmm6
		"psrld $4,%%xmm6\n"
		"pand %%xmm2,%%xmm6\n"
		"pshufb %%xmm0,%%xmm7\n"                //shuffle a2,b2,c2,d2->xmm7
		"psrld $2,%%xmm7\n"
		"pand %%xmm2,%%xmm7\n"
		"pshufb %%xmm0,%%xmm8\n"                //shuffle a3,b3,c3,d3->xmm8
		"pand %%xmm2,%%xmm8\n"
		...
		"movdqu %%xmm8,%3\n"                    //save output
				...
				//deal with e, f, g, h(almost same as a, b, c, d)
		"movdqa %%xmm15,%%xmm5\n"
		"movdqa %%xmm5,%%xmm6\n"                //save for future aligned move
		"movdqa %%xmm5,%%xmm7\n"
		"movdqa %%xmm5,%%xmm8\n"
...
/*
template<typename T>
void unpackIntelParallel2(T * des, const uint32_t *src, int blockSize) {
	int i;
...
static const struct shfl_3_0{	//a0-d3 -> a0-a3(0-1byte), c0-c3(4-5byte)
int8_t a[16];
}shfl_3_0 __attribute__((aligned(16))) = {{2, 3, 0xff, 0xff, 7, 0, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff},};
static const struct shfl_3_1{	//a0-d3 -> b0-b3(0-1byte), d0-d3(4-5byte)
int8_t a[16];
}shfl_3_1 __attribute__((aligned(16))) = {{1, 2, 0xff, 0xff, 6, 7, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff},};
static const struct shfl_3_2{	//e0-h3 -> e0-e3(0-1byte), g0-g3(4-5byte)
int8_t a[16];
}shfl_3_2 __attribute__((aligned(16))) = {{4, 5, 0xff, 0xff, 9, 10, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff},};
static const struct shfl_3_3{	//e0-h3 -> f0-f3(0-1yte), h0-h3(4-5byte)
int8_t a[16];
}shfl_3_3 __attribute__((aligned(16))) = {{11, 4, 0xff, 0xff, 8, 9, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff},};

static const struct and_3_0{
int16_t a[8];
}and_3_0 __attribute__((aligned(16))) = {{0x7,0,0x7,0,0x7,0,0x7,0},};

template<typename T>
void unpackIntelParallel3(T* dest, const uint32_t *src, int blockSize)
{
	__asm__("movdqa %0,%%xmm0\n"//shfl_0
			"movdqa %1,%%xmm1\n"//shfl_1
			"movdqa %2,%%xmm2\n"//shfl_2
			"movdqa %3,%%xmm3\n"//shfl_3
			"movdqa %4,%%xmm4\n"//and_0
			 ::"m" (shfl_3_0.a[0]),"m" (shfl_3_1.a[0]),"m" (shfl_3_2.a[0]),"m" (shfl_3_3.a[0]),"m" (and_3_0.a[0]));

	for (int i = 32; i <= blockSize; i += 32, src += 3,dest+=32)
	{
		__asm__ volatile("prefetchnta %0"::"m" (src[0]));
		__asm__("movdqu %8,%%xmm5\n"            		//src[0]->xmm5
				"movdqa %%xmm5,%%xmm11\n"				//save for future align
...
/*
static const struct mask_3_0{	//a0, c0, e0, g0 or a1, c1, e1, g1
int8_t a[16];
}mask_3_0 __attribute__((aligned(16))) = {{3,0xff,0xff,0xff,0,0xff,0xff,0xff,5,0xff,0xff,0xff,10,0xff,0xff,0xff},};
static const struct mask_3_1{	//a2, c2, e2, g2
int8_t a[16];
}mask_3_1 __attribute__((aligned(16))) = {{2,3,0xff,0xff,7,0,0xff,0xff,4,5,0xff,0xff,9,10,0xff,0xff},};
static const struct mask_3_2{	//a3, c3, e3, g3 or b0, d0, f0, h0
int8_t a[16];
}mask_3_2 __attribute__((aligned(16))) = {{2,0xff,0xff,0xff,7,0xff,0xff,0xff,4,0xff,0xff,0xff,9,0xff,0xff,0xff},};
static const struct mask_3_3{	//b1, d1, f1, h1
int8_t a[16];
}mask_3_3 __attribute__((aligned(16))) = {{1,2,0xff,0xff,6,7,0xff,0xff,11,4,0xff,0xff,8,9,0xff,0xff},};
static const struct mask_3_4{	//b2, d2, f2, h2 or b3, d3, f3, h3
int8_t a[16];
}mask_3_4 __attribute__((aligned(16))) = {{1,0xff,0xff,0xff,6,0xff,0xff,0xff,11,0xff,0xff,0xff,8,0xff,0xff,0xff},};
static const struct and_3_0{
int16_t a[8];
}and_3_0 __attribute__((aligned(16))) = {{0x7,0,0x7,0,0x7,0,0x7,0},};
...
void unpackIntelParallel3(T* dest, const uint32_t *src, int blockSize)
{
	__asm__("movdqa %0,%%xmm0\n"//mask_0
			...
			"movdqa %4,%%xmm4\n"//mask_4
			"movdqa %5,%%xmm6\n"//and_0
			 ::"m" (mask_3_0.a[0]),"m" (mask_3_1.a[0]),"m" (mask_3_2.a[0]),"m" (mask_3_3.a[0]),"m" (mask_3_4.a[0]),"m" (and_3_0.a[0]));
...
/*
template<typename T>
void unpackIntelParallel3(T * des, const uint32_t *src, int blockSize) {
	int i;
...
static const struct mask_4_0{
int8_t a[16];
}mask_4_0 __attribute__((aligned(16))) = {{3,0xff,0xff,0xff,1,0xff,0xff,0xff,7,0xff,0xff,0xff,5,0xff,0xff,0xff},};
static const struct mask_4_1{
int8_t a[16];
}mask_4_1 __attribute__((aligned(16))) = {{2,0xff,0xff,0xff,0,0xff,0xff,0xff,6,0xff,0xff,0xff,4,0xff,0xff,0xff},};
static const struct mask_4_2{
int8_t a[16];
}mask_4_2 __attribute__((aligned(16))) = {{11,0xff,0xff,0xff,9,0xff,0xff,0xff,15,0xff,0xff,0xff,13,0xff,0xff,0xff},};
static const struct mask_4_3{
int8_t a[16];
}mask_4_3 __attribute__((aligned(16))) = {{10,0xff,0xff,0xff,8,0xff,0xff,0xff,14,0xff,0xff,0xff,12,0xff,0xff,0xff},};
static const struct and_4_0{
int16_t a[8];
}and_4_0 __attribute__((aligned(16))) = {{0x0f,0,0x0f,0,0x0f,0,0x0f,0},};

template<typename T>
void unpackIntelParallel4(T* dest, const uint32_t *src, int blockSize)
{
	__asm__("movdqa %0,%%xmm4\n"//and_0
			...
			"movdqa %3,%%xmm2\n"//mask_2
			"movdqa %4,%%xmm3\n"//mask_3
			 ::"m" (and_4_0.a[0]),"m" (mask_4_0.a[0]),"m" (mask_4_1.a[0]),"m" (mask_4_2.a[0]),"m" (mask_4_3.a[0]));

	for (int i = 32; i <= blockSize; i += 32, src += 4,dest+=32)
	{
		__asm__ volatile("prefetchnta %0"::"m" (src[0]));
		__asm__("movdqu %4,%%xmm5\n"            //src[0]->xmm5
		"movdqa %%xmm5,%%xmm11\n"               //save for op of next 8 bytes
		"movdqa %%xmm5,%%xmm6\n"                //save for future aligned move
		"movdqa %%xmm5,%%xmm7\n"
		"movdqa %%xmm5,%%xmm8\n"
		"pshufb %%xmm0,%%xmm5\n"                //shuffle a0,b0,c0,d0->xmm5
...
/*
template<typename T>
void unpackIntelParallel4(T * des, const uint32_t *src, int blockSize) {
	int i;
...
static const struct shfl_5_0{	//a0-d3 -> a0-a3(0-2byte), c0-c3(4-6byte)
int8_t a[16];
}shfl_5_0 __attribute__((aligned(16))) = {{1, 2, 3, 0xff, 4, 5, 6, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff},};
static const struct shfl_5_1{	//a0-d3 -> b0-b3(0-2byte), d0-d3(4-6byte)
int8_t a[16];					//or e0-h3 -> e0-e3(0-2byte), g0-g3(4-6byte)
}shfl_5_1 __attribute__((aligned(16))) = {{7, 0, 1, 0xff, 10, 11, 4, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff},};
static const struct shfl_5_2{	//e0-h3 -> f0-f3(0-2byte), h0-h3(4-6byte)
int8_t a[16];
}shfl_5_2 __attribute__((aligned(16))) = {{5, 6, 7, 0xff, 8, 9, 10, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff},};

static const struct and_5_0{
int16_t a[8];
}and_5_0 __attribute__((aligned(16))) = {{0x1f,0,0x1f,0,0x1f,0,0x1f,0},};

template<typename T>
void unpackIntelParallel5(T* dest, const uint32_t *src, int blockSize)
{
	__asm__("movdqa %0,%%xmm0\n"//shfl_0
			"movdqa %1,%%xmm1\n"//shfl_1
			"movdqa %2,%%xmm2\n"//shfl_2
			"movdqa %3,%%xmm3\n"//and_0
			 ::"m" (shfl_5_0.a[0]),"m" (shfl_5_1.a[0]),"m" (shfl_5_2.a[0]),"m" (and_5_0.a[0]));

	for (int i = 32; i <= blockSize; i += 32, src += 5,dest+=32)
	{
		__asm__ volatile("prefetchnta %0"::"m" (src[0]));
		__asm__("movdqu %8,%%xmm5\n"            		//src[0]->xmm5
					//process a0-d3
...
/*
static const struct mask_5_0{	//a0, c0 or e0, g0
int8_t a[16];
}mask_5_0 __attribute__((aligned(16))) = {{3,0xff,0xff,0xff,6,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff},};
static const struct mask_5_1{	//a1, c1 or e1, g1
int8_t a[16];
}mask_5_1 __attribute__((aligned(16))) = {{2,3,0xff,0xff,5,6,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff},};
static const struct mask_5_2{	//a2, c2 or e2, g2
int8_t a[16];
}mask_5_2 __attribute__((aligned(16))) = {{2,0xff,0xff,0xff,5,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff},};
static const struct mask_5_3{	//a3, c3 or e3, g3
int8_t a[16];
}mask_5_3 __attribute__((aligned(16))) = {{1,2,0xff,0xff,4,5,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff},};
static const struct mask_5_4{	//b0, d0 or f0, h0
int8_t a[16];
}mask_5_4 __attribute__((aligned(16))) = {{0,1,0xff,0xff,11,4,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff},};
static const struct mask_5_5{	//b1, d1 or f1, h1
int8_t a[16];
}mask_5_5 __attribute__((aligned(16))) = {{0,0xff,0xff,0xff,11,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff},};
static const struct mask_5_6{	//b2, d2 or f2, h2
int8_t a[16];
}mask_5_6 __attribute__((aligned(16))) = {{7,0,0xff,0xff,10,11,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff},};
static const struct mask_5_7{	//b3, d3 or f3, h3
int8_t a[16];
}mask_5_7 __attribute__((aligned(16))) = {{7,0xff,0xff,0xff,10,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff},};
static const struct shfl_5_0{
int8_t a[16];
}shfl_5_0 __attribute__((aligned(16))) = {{6, 7, 0, 1, 10, 11, 4, 5, 0xff, 0xff, 8, 9, 0xff, 0xff, 0xff, 0xff},};
...
void unpackIntelParallel5(T* dest, const uint32_t *src, int blockSize)
{
	__asm__("movdqa %0,%%xmm0\n"//mask_0
			...
			"movdqa %7,%%xmm7\n"//mask_7
			"movdqa %8,%%xmm8\n"//mask_8
			 ::"m" (mask_5_0.a[0]),"m" (mask_5_1.a[0]),"m" (mask_5_2.a[0]),"m" (mask_5_3.a[0]),"m" (mask_5_4.a[0]),
			  "m" (mask_5_5.a[0]),"m" (mask_5_6.a[0]),"m" (mask_5_7.a[0]),"m" (shfl_5_0.a[0]));


	for (int i = 32; i <= blockSize; i += 32, src += 5,dest+=32)
	{
		__asm__ volatile("prefetchnta %0"::"m" (src[0]));
		__asm__("movdqu %8,%%xmm9\n"            		//src[0]->xmm9
				"movdqu %9,%%xmm10\n"					//src[2]->xmm10

				"pshufb %%xmm8,%%xmm10\n"               //shuffle xmm10, so the inner byte position of xmm10 will be the same as xmm9
...
/*
template<typename T>
void unpackIntelParallel5(T * des, const uint32_t *src, int blockSize) {
	int i;
...
template <typename T>
void unpackIntelParallel6(T* dest, const uint32_t *src, int blockSize)
{
	__asm__("movdqa %0,%%xmm4\n"//and_0
			"movdqa %1,%%xmm0\n"//mask_0
			"movdqa %2,%%xmm1\n"//mask_1
	...
	for (int i = 32; i <= blockSize; i += 32, src += 6,dest+=32)
	{
		__asm__ volatile("prefetchnta %0"::"m" (src[0]));
		__asm__("movdqu %4,%%xmm5\n"            //src[0]->xmm5
		"movdqa %%xmm5,%%xmm6\n"                //save for future aligned move
...
/*
template<typename T>
void unpackIntelParallel6(T * des, const uint32_t *src, int blockSize) {
	int i;
...
static const struct shfl_7_0{	//a0-d3 -> a0-a3(0-3byte), c0-c3(4-7byte)
int8_t a[16];
}shfl_7_0 __attribute__((aligned(16))) = {{0, 1, 2, 3, 9, 10, 11, 4, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff},};
static const struct shfl_7_1{	//a0-d3 -> b0-b3(0-3byte), d0-d3(4-7byte)
int8_t a[16];
}shfl_7_1 __attribute__((aligned(16))) = {{5, 6, 7, 0, 14, 15, 8, 9, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff},};
static const struct shfl_7_2{	//e0-h3 -> e0-e3(0-3byte), g0-g3(4-7byte)
int8_t a[16];
}shfl_7_2 __attribute__((aligned(16))) = {{6, 7, 0, 1, 15, 8, 9, 10, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff},};
static const struct shfl_7_3{	//e0-h3 -> f0-f3(0-3yte), h0-h3(4-7byte)
int8_t a[16];
}shfl_7_3 __attribute__((aligned(16))) = {{11, 4, 5, 6, 12, 13, 14, 15, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff},};

static const struct and_7_0{
int16_t a[8];
}and_7_0 __attribute__((aligned(16))) = {{0x7f,0,0x7f,0,0x7f,0,0x7f,0},};

template<typename T>
void unpackIntelParallel7(T* dest, const uint32_t *src, int blockSize)
{
	__asm__("movdqa %0,%%xmm0\n"//shfl_0
			"movdqa %1,%%xmm1\n"//shfl_1
			"movdqa %2,%%xmm2\n"//shfl_2
			"movdqa %3,%%xmm3\n"//shfl_3
			"movdqa %4,%%xmm4\n"//and_0
			 ::"m" (shfl_7_0.a[0]),"m" (shfl_7_1.a[0]),"m" (shfl_7_2.a[0]),"m" (shfl_7_3.a[0]),"m" (and_7_0.a[0]));

	for (int i = 32; i <= blockSize; i += 32, src += 7,dest+=32)
	{
		__asm__ volatile("prefetchnta %0"::"m" (src[0]));
		__asm__("movdqu %8,%%xmm5\n"            		//src[0]->xmm5
					//process a0-d3
...
/*
static const struct mask_7_0{	//a0, c0 or e0, g0
int8_t a[16];
}mask_7_0 __attribute__((aligned(16))) = {{3,0xff,0xff,0xff,4,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff},};
static const struct mask_7_1{	//a1, c1 or e1, g1
int8_t a[16];
}mask_7_1 __attribute__((aligned(16))) = {{2,3,0xff,0xff,11,4,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff},};
static const struct mask_7_2{	//a2, c2 or e2, g2
int8_t a[16];
}mask_7_2 __attribute__((aligned(16))) = {{1,2,0xff,0xff,10,11,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff},};
static const struct mask_7_3{	//a3, c3 or e3, g3
int8_t a[16];
}mask_7_3 __attribute__((aligned(16))) = {{0,1,0xff,0xff,9,10,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff},};
static const struct mask_7_4{	//b0, d0 or f0, h0
int8_t a[16];
}mask_7_4 __attribute__((aligned(16))) = {{7,0,0xff,0xff,8,9,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff},};
static const struct mask_7_5{	//b1, d1 or f1, h1
int8_t a[16];
}mask_7_5 __attribute__((aligned(16))) = {{6,7,0xff,0xff,15,8,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff},};
static const struct mask_7_6{	//b2, d2 or f2, h2
int8_t a[16];
}mask_7_6 __attribute__((aligned(16))) = {{5,6,0xff,0xff,14,15,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff},};
static const struct mask_7_7{	//b3, d3 or f3, h3
int8_t a[16];
}mask_7_7 __attribute__((aligned(16))) = {{5,0xff,0xff,0xff,14,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff},};
static const struct shfl_7_0{
int8_t a[16];
}shfl_7_0 __attribute__((aligned(16))) = {{6, 7, 0, 1, 10, 11, 4, 5, 14, 15, 8, 9, 0xff, 0xff, 12, 13},};
...
void unpackIntelParallel7(T* dest, const uint32_t *src, int blockSize)
{
	__asm__("movdqa %0,%%xmm0\n"//mask_0
			...
			"movdqa %7,%%xmm7\n"//mask_7
			"movdqa %8,%%xmm8\n"//mask_8
			 ::"m" (mask_7_0.a[0]),"m" (mask_7_1.a[0]),"m" (mask_7_2.a[0]),"m" (mask_7_3.a[0]),"m" (mask_7_4.a[0]),
			  "m" (mask_7_5.a[0]),"m" (mask_7_6.a[0]),"m" (mask_7_7.a[0]),"m" (shfl_7_0.a[0]));
...
/*
template<typename T>
void unpackIntelParallel7(T * des, const uint32_t *src, int blockSize) {
	int i;
...
static const struct mask_8_0{	//a0, a1, a2, a3
int8_t a[16];
}mask_8_0 __attribute__((aligned(16))) = {{3,0xff,0xff,0xff,2,0xff,0xff,0xff,1,0xff,0xff,0xff,0,0xff,0xff,0xff},};
static const struct mask_8_1{	//b0, b1, b2, b3
int8_t a[16];
}mask_8_1 __attribute__((aligned(16))) = {{7,0xff,0xff,0xff,6,0xff,0xff,0xff,5,0xff,0xff,0xff,4,0xff,0xff,0xff},};
static const struct mask_8_2{	//c0, c1, c2, c3
int8_t a[16];
}mask_8_2 __attribute__((aligned(16))) = {{11,0xff,0xff,0xff,10,0xff,0xff,0xff,9,0xff,0xff,0xff,8,0xff,0xff,0xff},};
static const struct mask_8_3{	//d0, d1, d2, d3
int8_t a[16];
}mask_8_3 __attribute__((aligned(16))) = {{15,0xff,0xff,0xff,14,0xff,0xff,0xff,13,0xff,0xff,0xff,12,0xff,0xff,0xff},};

template <typename T>
void unpackIntelParallel8(T* dest, const uint32_t *src, int blockSize)
{
	__asm__("movdqa %0,%%xmm0\n"//mask_0
			"movdqa %1,%%xmm1\n"//mask_1
			"movdqa %2,%%xmm2\n"//mask_2
			"movdqa %3,%%xmm3\n"//mask_3
			 ::"m" (mask_8_0.a[0]),"m" (mask_8_1.a[0]),"m" (mask_8_2.a[0]),"m" (mask_8_3.a[0]));

	for (int i = 32; i <= blockSize; i += 32, src += 8,dest+=32)
	{
		__asm__ volatile("prefetchnta %0"::"m" (src[0]));
		__asm__("movdqu %4,%%xmm5\n"            		//src[0]->xmm5
				"movdqa %%xmm5,%%xmm6\n"                //save for future aligned move
...
/*
template<typename T>
void unpackIntelParallel8(T * des, const uint32_t *src, int blockSize) {
	int i;
...
*/
template<typename T>
void unpackIntelParallel9(T * des, const uint32_t *src, int blockSize) {
	int i;
...
template<typename T>
void unpackIntelParallel10(T * des, const uint32_t *src, int blockSize) {
	int i;
...
template<typename T>
void unpackIntelParallel11(T * des, const uint32_t *src, int blockSize) {
	int i;
...
template<typename T>
void unpackIntelParallel12(T * des, const uint32_t *src, int blockSize) {
	int i;
...
template<typename T>
void unpackIntelParallel13(T * des, const uint32_t *src, int blockSize) {
	int i;
...
template<typename T>
void unpackIntelParallel14(T * des, const uint32_t *src, int blockSize) {
	int i;
...
template<typename T>
void unpackIntelParallel15(T * des, const uint32_t *src, int blockSize) {
	int i;
...
template<typename T>
void unpackIntelParallel16(T * des, const uint32_t *src, int blockSize) {
	int i;
...
template<typename T>
void unpackIntelParallel17(T * des, const uint32_t *src, int blockSize) {
	int i;
...
template<typename T>
void unpackIntelParallel18(T * des, const uint32_t *src, int blockSize) {
	int i;
...
template<typename T>
void unpackIntelParallel19(T * des, const uint32_t *src, int blockSize) {
	int i;
...
template<typename T>
void unpackIntelParallel20(T * des, const uint32_t *src, int blockSize) {
	int i;
...
template<typename T>
void unpackIntelParallel21(T * des, const uint32_t *src, int blockSize) {
	int i;
...
template<typename T>
void unpackIntelParallel22(T * des, const uint32_t *src, int blockSize) {
	int i;
...
template<typename T>
void unpackIntelParallel23(T * des, const uint32_t *src, int blockSize) {
	int i;
...
template<typename T>
void unpackIntelParallel24(T * des, const uint32_t *src, int blockSize) {
	int i;
...
template<typename T>
void unpackIntelParallel25(T * des, const uint32_t *src, int blockSize) {
	int i;
...
template<typename T>
void unpackIntelParallel26(T * des, const uint32_t *src, int blockSize) {
	int i;
...
template<typename T>
void unpackIntelParallel27(T * des, const uint32_t *src, int blockSize) {
	int i;
...
template<typename T>
void unpackIntelParallel28(T * des, const uint32_t *src, int blockSize) {
	int i;
...
template<typename T>
void unpackIntelParallel29(T * des, const uint32_t *src, int blockSize) {
	int i;
...
template<typename T>
void unpackIntelParallel30(T * des, const uint32_t *src, int blockSize) {
	int i;
...
template<typename T>
void unpackIntelParallel31(T * des, const uint32_t *src, int blockSize) {
	int i;
...
template<typename T>
void unpackIntelParallel32(T * des, const uint32_t *src, int blockSize) {
	int i;
...
#endif /* UNPACKINTELPARALLEL_HPP_ */
