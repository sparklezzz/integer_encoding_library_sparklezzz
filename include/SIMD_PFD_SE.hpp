/**
 * filename:SIMD_PFD_SE.hpp
 * @brief:
 * @data:2009-12-27
 * @author:shandd
 */

#ifndef SIMD_PFD_SE_HPP_
#define SIMD_PFD_SE_HPP_
...
class SIMD_PFD_SE: public PForDeltaBase {
public:
	SIMD_PFD_SE(float expRatio = 0.1);
...
template<typename T>
int SIMD_PFD_SE::encode(char* des, const T* src, uint32_t encodeNum) {
	int compLen = 0;
...
template<typename T>
int SIMD_PFD_SE::decode(T* des, const char* src, uint32_t decodeNum) {
	int decodeLen = 0;
...
template<typename T>
int SIMD_PFD_SE::encodeBlock(char* des, const T* src) {
	uint32_t expNum = 0;
	int compSize = 0;
...
template<typename T>
int SIMD_PFD_SE::decodeBlock(T* des, const char* src) {
...
template<typename T>
int SIMD_PFD_SE::pfor_encode(char *des, const T *src, int useBits,
		uint32_t& expNum) {
