...
#ifndef KSCHEME_1BIT_CU_HPP_
#define KSCHEME_1BIT_CU_HPP_
...
class KScheme_1Bit_CU: public Compressor {
public:
	KScheme_1Bit_CU() {
	}
	virtual ~KScheme_1Bit_CU() {
	}
...
template<typename T>
int KScheme_1Bit_CU::encode(char* des, const T* src, uint32_t encodeNum) {
	if (encodeNum < 4) {
...
template<typename T>
int KScheme_1Bit_CU::decode(T* des, const char* src, uint32_t decodeNum) {
	if (decodeNum < 4) {
...
template<typename T>
int KScheme_1Bit_CU::encodeBlock(char* des, const T* src, uint32_t encodeNum) {
...
template<typename T>
int KScheme_1Bit_CU::decodeBlock(T* des, const char* src, uint32_t decodeNum) {
...
int KScheme_1Bit_CU::decodeBlock(T* des, const char* src, uint32_t decodeNum) {
	...
	while (decoded < decodeNum) {
		// get a valid unary descriptor
		uint32_t bitCount = 0;
		while (((uint64_t)(*modeSegPos) >> controlBitOffset) & 0x1) {
			controlBitOffset ++;
			bitCount ++;
		}
		bitCount ++;
