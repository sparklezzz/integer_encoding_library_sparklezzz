/**
 * filename:SIMD_PFD_SVL.hpp
 * @brief:
 * @data:2009-12-27
 * @author:shandd
 */

#ifndef SIMD_PFD_SVL_HPP_
#define SIMD_PFD_SVL_HPP_
...
class SIMD_PFD_SVL: public PForDeltaBase {
public:
	SIMD_PFD_SVL(float expRatio = 0.1);
...
template<typename T>
int SIMD_PFD_SVL::encode(char* des, const T* src, uint32_t encodeNum) {
	int compLen = 0;
...
template<typename T>
int SIMD_PFD_SVL::decode(T* des, const char* src, uint32_t decodeNum) {
	int decodeLen = 0;
...
template<typename T>
int SIMD_PFD_SVL::encodeBlock(char* des, const T* src) {
	uint32_t expNum = 0;
	int compSize = 0;
...
template<typename T>
int SIMD_PFD_SVL::decodeBlock(T* des, const char* src) {
...
template<typename T>
int SIMD_PFD_SVL::pfor_encode(char *des, const T *src, int useBits,
		uint32_t& expNum) {
	...
	if (expNum <= m_expRatio * m_blockSize) {
		...
		compSize += needWord * sizeof(uint32_t);
                if (expBit == 8) {
                        ...
                        simd_pack_se16<uint32_t>(desInt, expNumArr, expNum);
                }
                else {
