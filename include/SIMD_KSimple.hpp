...
#ifndef SIMD_KSIMPLE_HPP_
#define SIMD_KSIMPLE_HPP_
...
class SIMD_KSimple: public Compressor {
public:
	SIMD_KSimple() {
	}
	virtual ~SIMD_KSimple() {
	}
...
template<typename T>
int SIMD_KSimple::encode(char* des, const T* src, uint32_t encodeNum) {
	if (encodeNum < 16) {
...
template<typename T>
int SIMD_KSimple::decode(T* des, const char* src, uint32_t decodeNum) {
	if (decodeNum < 16) {
...
template<typename T>
int SIMD_KSimple::encodeBlock(char* des, const T* src, uint32_t encodeNum) {
...
template<typename T>
int SIMD_KSimple::decodeBlock(T* des, const char* src, uint32_t decodeNum) {
...
template<typename T>
int SIMD_KSimple::genModeArr(char* des, const T* maxQuadIntArr, uint8_t* modeArr,
		uint32_t quadEncodeNum) {
	int totalModeNum = 0;
...
template<typename T>
int SIMD_KSimple::encodeAllGroups(char* des, const T* src, uint32_t encodeNum,
		const uint8_t* modeArr, uint32_t totalModeNum) {
...
template<typename T>
int SIMD_KSimple::decodeSingleGroup(T* des, const uint32_t* src, uint8_t mode) {
