/*
 * SIMD_KScheme_4Bit_CU_opt.hpp
 *
 *  SIMD_KScheme_4Bit_Complete Unary
 *
 *  Created on: 2013-3-2
 *      Author: zxd
 */

#ifndef SIMD_KSCHEME_4BIT_CU_OPT_HPP_
#define SIMD_KSCHEME_4BIT_CU_OPT_HPP_
...
#include <immintrin.h>
#include <nmmintrin.h>
#include "simd_kscheme_4bit_cu_unpack.hpp"
...
class SIMD_KScheme_4Bit_CU_opt: public Compressor {
public:
	SIMD_KScheme_4Bit_CU_opt() {
	}
	virtual ~SIMD_KScheme_4Bit_CU_opt() {
	}
...
public:
	...
	virtual std::string getCompressorName() {
		return "SIMD_KScheme_4Bit_CU_opt";
	}
...
template<typename T>
int SIMD_KScheme_4Bit_CU_opt::encode(char* des, const T* src, uint32_t encodeNum) {
	if (encodeNum < 4) {
...
template<typename T>
int SIMD_KScheme_4Bit_CU_opt::decode(T* des, const char* src, uint32_t decodeNum) {
	if (decodeNum < 4) {
...
template<typename T>
int SIMD_KScheme_4Bit_CU_opt::encodeBlock(char* des, const T* src, uint32_t encodeNum) {
...
template<typename T>
int SIMD_KScheme_4Bit_CU_opt::decodeBlock(T* des, const char* src, uint32_t decodeNum) {
	...
	simd_kscheme_4bit_cu_unpack_prepare<T>();
	...
	while (decoded < decodeNum) {
		...
		const SIMDK4CUUnpackInfo &unpackInfo = SIMDK4CUUnpackInfoArr[*modeSegPos];
...
#endif /* SIMD_KSCHEME_4BIT_CU_OPT_HPP_ */
