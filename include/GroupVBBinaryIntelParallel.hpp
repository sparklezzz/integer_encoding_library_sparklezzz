/*
 * GroupVBBinaryIntelParallel.hpp
 *
 *  Created on: 2013-1-23
 *      Author: zxd
 */

#ifndef GROUPVBBINARYINTELPARALLEL_HPP_
#define GROUPVBBINARYINTELPARALLEL_HPP_

#ifdef USE_SSE_INSTRUCTION

#include "Compressor.hpp"
#include "GvintUnpack.hpp"
#include "GvintBinaryUnpackIntelParallel.hpp"
#include "VarByte.hpp"

namespace paradise {
namespace index {

class GroupVBBinaryIntelParallel: public Compressor {
public:
	GroupVBBinaryIntelParallel() {
	}
	virtual ~GroupVBBinaryIntelParallel() {
	}
...
private:
	...
	//overload
	static int decodeBlock(uint32_t* des, const char* srcInt, uint32_t decodeNum);

};

template<typename T>
int GroupVBBinaryIntelParallel::encode(char* des, const T* src, uint32_t encodeNum) {
	int compLen = 0;
...
template<typename T>
int GroupVBBinaryIntelParallel::decode(T* des, const char* src, uint32_t decodeNum) {
	int decodeLen = 0;
...
template<typename T>
int GroupVBBinaryIntelParallel::encodeBlock(char* des, const T* src, uint32_t encodeNum) {
	int compSize = 0;
	uint8_t maskByte;
	int groupLen = 0;
	uint8_t buffer[16];
...
template<typename T>
int GroupVBBinaryIntelParallel::decodeBlock(T* des, const char* src, uint32_t decodeNum) {
...
#endif /* GROUPVBBINARYINTELPARALLEL_HPP_ */
