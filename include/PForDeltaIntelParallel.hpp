/**
 * filename:PForDeltaIntelParallel.hpp
 * @brief:
 * @data:2009-12-27
 * @author:shandd
 */

#ifndef PForDeltaIntelParallel_HPP_
#define PForDeltaIntelParallel_HPP_
...
class PForDeltaIntelParallel: public PForDeltaBase {
public:
	PForDeltaIntelParallel(float expRatio = 0.1);
...
template<typename T>
int PForDeltaIntelParallel::encode(char* des, const T* src, uint32_t encodeNum) {
	int compLen = 0;
...
template<typename T>
int PForDeltaIntelParallel::decode(T* des, const char* src, uint32_t decodeNum) {
	int decodeLen = 0;
...
template<typename T>
int PForDeltaIntelParallel::encodeBlock(char* des, const T* src) {
	uint32_t expNum = 0;
	int compSize = 0;
...
template<typename T>
int PForDeltaIntelParallel::decodeBlock(T* des, const char* src) {
...
template<typename T>
int PForDeltaIntelParallel::pfor_encode(char *des, const T *src, int useBits,
		uint32_t& expNum) {
...
#endif /* PForDeltaIntelParallel_HPP_ */
