/*
 * KScheme_3Bit_Binary.hpp
 *
 *  Created on: 2013-3-2
 *      Author: zxd
 */

#ifndef KSCHEME_3BIT_BINARY_HPP_
#define KSCHEME_3BIT_BINARY_HPP_
...
class KScheme_3Bit_Binary: public Compressor {
public:
	KScheme_3Bit_Binary() {
	}
	virtual ~KScheme_3Bit_Binary() {
	}
...
template<typename T>
int KScheme_3Bit_Binary::encode(char* des, const T* src, uint32_t encodeNum) {
	if (encodeNum < 4) {
...
template<typename T>
int KScheme_3Bit_Binary::decode(T* des, const char* src, uint32_t decodeNum) {
	if (decodeNum < 4) {
...
template<typename T>
int KScheme_3Bit_Binary::encodeBlock(char* des, const T* src, uint32_t encodeNum) {
	...
	*desInt = 4 + (quadEncodeNum + 1) / 2;	// record the begin position of data segment
	...
	for (uint32_t i=0; i<encodeNum; i+=4) {
		...
		if (quadMaxInt == 0)
			quadMaxInt ++;
		char threeBitCount = 0;
		while (quadMaxInt > 0) {
			quadMaxInt >>= 3;
			threeBitCount ++;
		}
		*modeSegPos |= (threeBitCount-1) << ((quadIdx & 0x1) << 2);
		quadIdx ++;
		if (!(quadIdx & 0x1)) {
			modeSegPos ++;
		}

		uint32_t bitCount = threeBitCount * 3 < 32 ? threeBitCount * 3 : 32;
...
template<typename T>
int KScheme_3Bit_Binary::decodeBlock(T* des, const char* src, uint32_t decodeNum) {
	...
	while (decoded < decodeNum) {
		for (int i=0; i<2 && decoded < decodeNum; i++) {
			uint32_t threeBitCount = 1 + (((*modeSegPos) >> (i<<2)) & 0xf);
			uint32_t bitCount = threeBitCount * 3 < 32 ? threeBitCount * 3 : 32;
			uint32_t bitLeftInUInt = 32 - curBitOffset;
			for	(uint32_t k=0; k<4; k++) {
...
#endif /* KSCHEME_3BIT_BINARY_HPP_ */
