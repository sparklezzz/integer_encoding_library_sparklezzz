/**
 * filename:SIMD_PFD_VL.hpp
 * @brief:
 * @data:2009-12-27
 * @author:shandd
 */

#ifndef SIMD_PFD_VL_HPP_
#define SIMD_PFD_VL_HPP_
...
class SIMD_PFD_VL: public PForDeltaBase {
public:
	SIMD_PFD_VL(float expRatio = 0.1);
...
//public:
	//for statistics
	//static uint64_t simd_unpackCountArr[33];
...
template<typename T>
int SIMD_PFD_VL::encode(char* des, const T* src, uint32_t encodeNum) {
	int compLen = 0;
...
template<typename T>
int SIMD_PFD_VL::decode(T* des, const char* src, uint32_t decodeNum) {
	int decodeLen = 0;
...
template<typename T>
int SIMD_PFD_VL::encodeBlock(char* des, const T* src) {
	uint32_t expNum = 0;
	int compSize = 0;
...
template<typename T>
int SIMD_PFD_VL::decodeBlock(T* des, const char* src) {
...
template<typename T>
int SIMD_PFD_VL::pfor_encode(char *des, const T *src, int useBits,
		uint32_t& expNum) {
