...
class PFD_VL: public PForDeltaBase {
public:
	PFD_VL(float expRatio = 0.1);
...
template<typename T>
int PFD_VL::encode(char* des, const T* src, uint32_t encodeNum) {
	int compLen = 0;
...
template<typename T>
int PFD_VL::decode(T* des, const char* src, uint32_t decodeNum) {
	int decodeLen = 0;
...
template<typename T>
int PFD_VL::encodeBlock(char* des, const T* src) {
	uint32_t expNum = 0;
	int compSize = 0;
...
template<typename T>
int PFD_VL::decodeBlock(T* des, const char* src) {
...
template<typename T>
int PFD_VL::pfor_encode(char *des, const T *src, int useBits,
		uint32_t& expNum) {
